...
void loop() {      // anything starting with "//" is just a comment :)
  ...
  fireLaser(1);       // Fires laser (x) times
  ////// end of your code
...
const int stopL = 88;    // 90 is usually "stopped" (if inching CW, tune >90, if inching CCW, tune <90 (for dead stop))
const int stopR = 89;    // 90 is usually "stopped" (if inching CW, tune >90, if inching CCW, tune <90 (for dead stop))
